...
const cloudinaryWatermarked = (publicId) => {
  ...
  const transforms = [
    ...
    "l_beerfest26_logo",
    "w_150,o_35,g_south_east,x_15,y_15",
    "fl_layer_apply",
  ].join("/");
...
const cloudinaryThumb = (publicId) => {
  return `https://res.cloudinary.com/${CLOUDINARY_CLOUD}/image/upload/w_300,h_300,c_fill,q_auto,f_auto/l_beerfest26_logo/w_60,o_35,g_south_east,x_6,y_6/fl_layer_apply/${publicId}`;
};
            ...
            {photos.map(photo => {
              ...
              return (
                <div key={photo.publicId} style={{ position:"relative",aspectRatio:"1",borderRadius:8,overflow:"hidden",border:`2px solid ${isOwner?B.gold:B.tealLight}`,cursor:"pointer" }}>
                  ...
                  />
                  {/* Only show delete button for photos this user uploaded */}
                  {isOwner && (
                    <button onClick={e=>{e.stopPropagation();remove(photo.publicId);}}
                      style={{ position:"absolute",top:4,right:4,width:22,height:22,borderRadius:"50%",background:"rgba(217,75,53,0.85)",border:"none",color:"white",fontSize:11,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900 }}>✕</button>
                  )}
                  {isOwner && (
                    <div style={{ position:"absolute",top:4,left:4,background:"rgba(196,154,60,0.9)",borderRadius:4,padding:"1px 5px",fontFamily:DISPLAY,fontWeight:800,fontSize:8,color:"white",letterSpacing:0.5 }}>YOURS</div>
                  )}
...
function BeerSetupPanel({ onBack }) {
  ...
  const [adminPhotos, setAdminPhotos] = useState(loadCloudPhotos());

  const deleteAdminPhoto = (publicId, uploaderName) => {
    if (!window.confirm(`Remove this photo by ${uploaderName}?`)) return;
    setAdminPhotos(prev => { const u = prev.filter(p => p.publicId !== publicId); saveCloudPhotos(u); return u; });
  };
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: photos.length > 0 ? 14 : 0 }}>
                <div>
                  ...
                  <div style={{ fontFamily:BODY, fontSize:12, color:B.tealMid, marginTop:2 }}>
                    {photos.length > 0 ? `${photos.length} photo${photos.length!==1?"s":""} uploaded to Cloudinary` : "No photos uploaded yet"}                  </div>
                </div>
                {photos.length > 0 && (
                  <button
                    onClick={() => {
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6 }}>
                  {photos.map((photo, i) => (
                    <div key={photo.publicId} style={{ position:"relative", aspectRatio:"1", borderRadius:6, overflow:"hidden", border:`1px solid ${B.tealLight}` }}>
                      ...
                      <button
                        onClick={() => deleteAdminPhoto(photo.publicId, photo.uploaderName)}
                        style={{ position:"absolute",top:3,right:3,width:18,height:18,borderRadius:"50%",background:"rgba(217,75,53,0.85)",border:"none",color:"white",fontSize:9,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900 }}>✕</button>
...
export default function BeerFestApp() {
  ...
  // Reload vendors + beer list from localStorage whenever admin closes
  // so attendees always see the latest data without refreshing the page
  ...
  // Also pick up changes if admin is running in a separate tab
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === VENDORS_KEY) setVendors(loadVendors());
      if (e.key === BEERS_KEY)   setBeerList(loadBeerList());
    };
  ...
  if (showAdmin) return <BeerSetupPanel onBack={handleAdminClose} />;
