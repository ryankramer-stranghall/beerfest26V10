...
function PhotoUpload({ uploaderName="", uploaderEmail="" }) {
  ...
  const uploadToCloudinary = async (file) => {
    ...
    if (uploaderName) fd.append("context", `uploader=${uploaderName}`);
  ...
  const handleFiles = async (files) => {
    ...
    setUploading([]);
    const newPhotos = results.filter(r=>r.status==="fulfilled").map(r=>({
      publicId: r.value,
      takenAt: new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),
      uploaderName: uploaderName||"Guest",
      uploaderEmail: uploaderEmail||"",
    }));
    setPhotos(prev => { const u=[...newPhotos,...prev]; saveCloudPhotos(u); return u; });
  };
  ...
  return (
    <div style={{ width:"100%",maxWidth:400,marginTop:24 }}>
      <div style={{ background:B.cream,border:`2px solid ${B.tealLight}`,borderRadius:14,padding:"20px 20px 22px" }}>
        ...
        <div style={{ fontFamily:BODY,fontSize:11,color:B.tealMid,opacity:0.6,marginBottom:14 }}>Photos upload to the cloud — visible to everyone ☁️</div>
        <div style={{ display:"flex",gap:10,marginBottom:16 }}>
          <button onClick={()=>cameraRef.current?.click()} disabled={uploading.length>0}
            style={{ flex:1,padding:"10px 0",borderRadius:8,background:uploading.length>0?"#aaa":B.red,border:"none",color:B.cream,fontFamily:DISPLAY,fontWeight:900,fontSize:14,textTransform:"uppercase",cursor:uploading.length>0?"not-allowed":"pointer",boxShadow:uploading.length>0?"none":`0 3px 12px ${B.red}44` }}>
            {uploading.length>0?"Uploading…":"📷 Take Photo"}
          </button>
          <button onClick={()=>fileRef.current?.click()} disabled={uploading.length>0}
            style={{ flex:1,padding:"10px 0",borderRadius:8,background:uploading.length>0?"#aaa":B.tealDark,border:"none",color:B.cream,fontFamily:DISPLAY,fontWeight:900,fontSize:14,textTransform:"uppercase",cursor:uploading.length>0?"not-allowed":"pointer" }}>
        ...
        {uploading.length>0 && (
          <div style={{ background:B.tealPale,borderRadius:8,padding:"10px 14px",marginBottom:14,fontFamily:BODY,fontSize:12,color:B.tealDark,display:"flex",alignItems:"center",gap:8 }}>
        ...
        {photos.length>0 ? (
            ...
            {photos.map(photo => {
              ...
              return (
                <div key={photo.publicId} style={{ position:"relative",aspectRatio:"1",borderRadius:8,overflow:"hidden",border:`2px solid ${isOwner?B.gold:B.tealLight}`,cursor:"pointer" }}>
                  <img src={cloudinaryThumb(photo.publicId)} alt="event photo" onClick={()=>setLightbox(photo.publicId)}
                  ...
                  <div style={{ position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(transparent,rgba(0,0,0,0.55))",padding:"4px 5px" }}>
                    ...
                    {isOwner && <div style={{ color:B.gold,fontSize:8,fontFamily:DISPLAY,fontWeight:800 }}>YOURS</div>}
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ border:`2px dashed ${B.tealLight}`,borderRadius:10,padding:"20px 16px",textAlign:"center",color:B.tealMid,fontFamily:BODY,fontSize:13,opacity:0.6 }}>No photos yet — snap some memories!</div>
        )}
      </div>
      {lightbox && (() => {
