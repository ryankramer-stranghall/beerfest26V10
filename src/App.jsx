...
const cloudinaryWatermarked = (publicId) => {
  ...
  const t = ["w_800,c_limit,q_auto,f_auto","l_beerfest26_logo","w_150,o_35,g_south_east,x_15,y_15","fl_layer_apply"].join("/");
  return `${base}/${t}/${publicId}`;
};
const cloudinaryThumb = (publicId) =>
  `https://res.cloudinary.com/${CLOUDINARY_CLOUD}/image/upload/w_300,h_300,c_fill,q_auto,f_auto/l_beerfest26_logo/w_60,o_35,g_south_east,x_6,y_6/fl_layer_apply/${publicId}`;
...
/* ─────────────────────────────────────────────
   PHOTO UPLOAD
───────────────────────────────────────────── */
function PhotoUpload({ uploaderName="", uploaderEmail="" }) {
  ...
  // Also subscribe to Firestore photos so cross-device photos appear
  useEffect(() => {
    const PHOTOS_DOC = doc(db, "beerfest26", "photos");
    const unsub = onSnapshot(PHOTOS_DOC, (snap) => {
      if (snap.exists() && snap.data().list) {
        const merged = snap.data().list;
        setPhotos(merged);
        saveCloudPhotos(merged);
      }
    });
    return unsub;
  }, []);
  ...
  const handleFiles = async (files) => {
    ...
    try {
      ...
      if (failed.length) setUploadError(`${failed.length} photo(s) failed to upload. Check your connection.`);
      ...
      if (succeeded.length) {
        ...
        setPhotos(prev => {
          ...
          // Save to Firestore so all devices see new photos
          const PHOTOS_DOC = doc(db, "beerfest26", "photos");
          setDoc(PHOTOS_DOC, { list: updated }, { merge: false }).catch(()=>{});
          return updated;
        });
      }
    } catch(e) {
  ...
  const remove = (publicId) => {
    setPhotos(prev => {
      const updated = prev.filter(p=>p.publicId!==publicId);
      saveCloudPhotos(updated);
      const PHOTOS_DOC = doc(db, "beerfest26", "photos");
      setDoc(PHOTOS_DOC, { list: updated }, { merge: false }).catch(()=>{});
      return updated;
  ...
  return (
    <div style={{ width:"100%",maxWidth:400,marginTop:24 }}>
      <div style={{ background:B.cream,border:`2px solid ${B.tealLight}`,borderRadius:14,padding:"20px 20px 22px" }}>
        ...
        <div style={{ fontFamily:BODY,fontSize:11,color:B.tealMid,opacity:0.6,marginBottom:14 }}>Photos upload to the cloud & visible on all devices ☁️</div>

        <div style={{ display:"flex",gap:10,marginBottom:12 }}>
          {/* Camera — NO multiple so capture works reliably on mobile */}
          ...
          {/* Gallery — multiple is fine here */}
          <button onClick={()=>{ setUploadError(""); fileRef.current?.click(); }} disabled={uploading}
            style={{ flex:1,padding:"11px 0",borderRadius:8,background:uploading?"#aaa":B.tealDark,border:"none",color:B.cream,fontFamily:DISPLAY,fontWeight:900,fontSize:14,textTransform:"uppercase",cursor:uploading?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6 }}>
            🖼 Upload
          </button>
        </div>

        {/* Camera — NO multiple attribute for mobile compatibility */}
        <input ref={cameraRef} type="file" accept="image/*" capture="environment"
          style={{display:"none"}} onChange={e=>{ handleFiles(e.target.files); e.target.value=""; }} />
        {/* Gallery picker — multiple ok */}
        <input ref={fileRef} type="file" accept="image/*" multiple
          style={{display:"none"}} onChange={e=>{ handleFiles(e.target.files); e.target.value=""; }} />

        {uploadError && (
          <div style={{ background:"#fee",border:`1px solid ${B.red}44`,borderRadius:8,padding:"8px 12px",marginBottom:12,fontFamily:BODY,fontSize:12,color:B.red }}>
            ⚠️ {uploadError}
          </div>
        )}
                ...
                <div key={photo.publicId} style={{ position:"relative",aspectRatio:"1",borderRadius:8,overflow:"hidden",border:`2px solid ${isOwner?B.gold:B.tealLight}`,cursor:"pointer" }}>
                  ...
                  {isOwner && (
                    <button onClick={e=>{e.stopPropagation();remove(photo.publicId);}}
                      style={{ position:"absolute",top:4,right:4,width:22,height:22,borderRadius:"50%",background:"rgba(0,0,0,0.65)",border:"none",color:"white",fontSize:11,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",lineHeight:1 }}>✕</button>
                  )}
      ...
      {/* Lightbox — always shows watermarked version */}
      {lightbox && (() => {
        ...
        return photo ? (
          <div onClick={()=>setLightbox(null)} style={{ position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.93)",display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
            <img
              src={cloudinaryWatermarked(photo.publicId)}
              alt="event photo"
...
export default function BeerFestApp() {
  ...
  // Subscribe to event doc on mount — stays live the whole time
  useEffect(() => {
  ...
  if (screen==="welcome") return (
    <div style={{ minHeight:"100vh",background:`linear-gradient(170deg,${B.tealDark} 0%,${B.tealMid} 55%,${B.tealLight} 100%)`,fontFamily:BODY,display:"flex",flexDirection:"column",alignItems:"center",padding:"0 20px 60px" }}>
      ...
      <PhotoUpload uploaderName={userName} uploaderEmail={userEmail} />
